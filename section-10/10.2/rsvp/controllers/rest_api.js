...
var RestAPI = function(config) {

  var self = this,
    Event = config.models.eventModel,
    ...
    saveEvent = function(o, callback) {
      ...
      if (typeof o.id === "undefined") {
        new Event(eventObj).save(function(err, result) {
          if (result) msg = { type: "new event" };
  ...
  self.create_event = function(req, res) {
    ...
    if (method === "get") {
      ...
      saveEvent({ body: req.body }, function(err, result, msg) {
        if (err) {
          options.redirect = err.errors ? "" : "/events";
          return handleError(err, req, res, options);
        }
        if (msg) self.emit(msg.type, msg);
  ...
  self.update_event = function(req, res) {
    ...
    if (method === "get") {
      ...
      saveEvent({ id: options.eventId, body: req.body }, function(err, result, msg) {
        if (err) {
          options.redirect = err.errors ? "" : "/events";
          return handleError(err, req, res, options);
        }
  ...
  self.show_event = function(req, res, next) {
  ...
  self.list_events = function(req, res) {
  ...
  self.destroy_event = function(req, res) {
  ...
  self.destroy_response = function(req, res) {
